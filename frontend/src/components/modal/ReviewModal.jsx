...
export const ReviewModal = (props) => {
  ...
  const [evaluation, setEvaluation] = useState(isEditMode ? review.evaluation : 3);
  const [reviewImageFile, setLocalReviewImage] = useState(null);
  const [preview, setPreview] = useState(isEditMode ? (review.image_url || '') : '');
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      setPreview(isEditMode ? (review.image_url || '') : '');
  ...
  const handleRemoveImage = (e) => {
    ...
    const fileInput = document.getElementById('review_image_input');
  ...
  return (
    <form onSubmit={handleSubmit} onChange={checkDirty} className="bg-white p-6 md:p-8">
      <div className="max-w-lg mx-auto">
        ...
        {localError && <p className="mb-4 text-sm font-bold text-red-500 bg-red-50 p-3 rounded-lg">{localError}</p>}
        ...
        <div className="mb-8">
          ...
          <textarea 
            ...
            defaultValue={isEditMode ? review.content : ''}
          ></textarea>
        </div>
