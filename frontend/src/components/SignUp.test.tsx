...
describe('SignUp Component', () => {
  ...
  test('バリデーション: ニックネームが10文字を超えるとエラーが表示されること', async () => {
    ...
    const submitButton = screen.getByRole('button', { name: '送信する' });
    const form1 = submitButton.closest('form');
    ...
    const registerButton = screen.getByRole('button', { name: '登録して始める' });
    const form2 = registerButton.closest('form');
  ...
  test('正しく入力するとFirebase登録とAPI送信が行われ、トップへ遷移すること', async () => {
    ...
    (postCreateUser as jest.Mock).mockResolvedValueOnce({ id: 1, name: 'テストユーザー' });
